...
def create_asset(*, organization, user, display_name, default_role, upload) -> BrandAsset:
    ...
    add_asset_version(asset=asset, organization=organization, user=user, upload=upload)
    return asset
...
def add_asset_version(*, asset, organization, user, upload) -> BrandAssetVersion:
    ...
    number = (
        asset.versions.select_for_update().aggregate(maximum=models.Max("number"))["maximum"] or 0
    ) + 1
