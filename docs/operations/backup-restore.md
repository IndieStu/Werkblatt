...
Secrets und die produktive `.env` gehören nicht in Git. Die Zielinfrastruktur muss Secrets verschlüsselt und zugriffsbeschränkt sichern; Zugriff, Rotation und Wiederherstellung der Credentials bleiben gesondert zu protokollieren. Die Deployment-Konfiguration wird getrennt versioniert.
...
Ein infrastruktureller Restore-Test darf niemals in die produktive Werkblatt-Datenbank schreiben. Er verwendet eine isolierte temporäre Datenbank, ein temporäres Medienverzeichnis und das exakt zu prüfende Container-Image. Nach erfolgreicher Prüfung werden Anzahl und Hashes der synthetischen Snapshots sowie die Abrufbarkeit der synthetischen PDF-Datei verglichen. Konkrete Speicherpfade, Backupwerkzeuge und Aufbewahrungsregeln gehören in die Dokumentation der jeweiligen Zielinfrastruktur.
